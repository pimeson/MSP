...
app.controller('adminCtrl', function ($scope, projectFactory, projects, Upload, $state, downloads, fileFactory) {
  ...
  $scope.uploadAboutHtml = (file) => {
    Upload.upload({
      url: 'https://matthewspiegelman.herokuapp.com/api/about/aboutHtml',
      data: {
        file: file
      }
    }).then((res) => {
  ...
  $scope.uploadPortrait = (file) => {
    Upload.upload({
      url: 'https://matthewspiegelman.herokuapp.com/api/about/aboutPortrait',
      data: {
        file: file
      }
    }).then((res) => {
  ...
  $scope.newDownload = (file) => {
    ...
    if ($scope.dlTitle) {
      Upload.upload({
        url: 'https://matthewspiegelman.herokuapp.com/api/about/upload',
        data: {
          file: file,
          title: $scope.dlTitle
        }
